...
import { MessageSquare, Phone, Calendar, Code, TrendingUp, Megaphone, BarChart3, Globe, Gavel } from 'lucide-react';
import { useUserProfile } from '../hooks/useUserProfile';
import { useCache } from '../hooks/useCache';
import { BinanceClient } from '../negociacoes/services/binance-api';
import { formatPrice, formatPercent } from '../negociacoes/services/binance-api';
...
function Dashboard({ onNavigateToAccount, onNavigateToWhatsapp, onNavigateToWhatsAppSetup, onNavigateToDialer, onNavigateToScheduler, onNavigateToFranchises, onNavigateToWebsites, onNavigateToLandingPages }: DashboardProps) {
  ...
  const plans: Plan[] = [
    ...
    // OCULTADO: Plano Sistema de Agendamentos (id: '2')
    // {
    //   id: '2',
  ...
  useEffect(() => {
    ...
    const services: Service[] = [
      ...
      },
      {
        ...
        // OCULTADO TEMPORARIAMENTE: Bloqueio ignorado
        isActive: false, // TEMPORARIAMENTE: sempre ativo — original: (profile?.ligacoes_active || false) && !profile?.ligacoes_access_blocked
      },
      {
        id: '3',
        name: 'Sistema de Agendamentos',
        description: 'Sistema de agendamentos com controle de unidade, profissionais e serviços.',
        icon: <Calendar className="w-8 h-8" />,
        // isActive: profile?.agendamentos_active || false,
        isActive: true, // Liberado a pedido do usuário
      },
      {
        id: '4',
      ...
      },
      {
        ...
        isActive: (profile?.marketing_active || false || profile?.trial_marketing_active || false) && !profile?.marketing_access_blocked,
      },
      {
        id: '6',
      ...
      {
        ////id: '7',
        name: 'Agente para Advogados',
        description: 'Websites profissionais construídos dentro do Código de Ética da OAB',
  ...
  useEffect(() => {
    ...
    const fetchPrices = async () => {
      try {
        ...
        const tickers = await Promise.all(symbols.map(s => binance.getTicker(s)));

        // Cotação aproximada do Dólar para BRL
        const usdToBrl = 5.05;

        const mapped = tickers.map(t => ({
          symbol: t.symbol.replace('USDT', ''),
          price: t.price * usdToBrl,
          changePercent: t.changePercent24h,
          name: t.symbol === 'BTCUSDT' ? 'Bitcoin' :
            t.symbol === 'ETHUSDT' ? 'Ethereum' :
              t.symbol === 'SOLUSDT' ? 'Solana' : 'BNB'
        }));
        setCryptoPrices(mapped);
      } catch (err) {
        console.error('Erro ao buscar preços Binance:', err);
      } finally {
  ...
  const handleConfigure = (id: string) => {
    ...
    if (id === '7') {
      // Redirecionar para página de gerenciamento de websites, filtrando para advogados
      // A página de gerenciamento (`/websites`) continuará usando as tabelas de `user_websites`,
      // mas receberá um query param para mostrar apenas websites do tipo 'lawyer'.
      window.location.href = '/websites?websiteType=lawyer';
      return;
    }
  ...
  const generateComboPlan = (): Plan => {
    const comboFeatures = [
      ...
      ...(selectedComboServices.includes('3') ? ['Sistema de Agendamentos', 'Agendamento inteligente via WhatsApp'] : []),
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 auto-rows-fr">
                {servicesState.map(service => {
                  const isContracted =
                    ...
                    service.id === '7' ||
                    service.id === '4' ||
                    (service.id === '5' && (profile?.marketing_active || profile?.trial_marketing_active) && !profile?.marketing_access_blocked);
