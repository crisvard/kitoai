...
import { Play, Pause, Square, Phone, Users, Clock, TrendingUp } from 'lucide-react';
import { useDialer } from '../contexts/DialerContext';

const Dashboard: React.FC = () => {
  const { state, startCampaign, stopCampaign, pauseCampaign } = useDialer();
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-br from-white/5 to-white/[0.02] backdrop-blur-xl border border-white/10 rounded-3xl p-8">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className={`px-4 py-2 rounded-full text-sm font-medium ${isRunning
          ...
          <div className="mb-4">
            ...
            <div className="w-full bg-gray-700 rounded-full h-3">
              <div
                ...
                style={{
                  width: `${currentCampaign.totalCalls > 0
                ...
                .map((contact) => (
                  <div key={contact.id} className="flex items-center justify-between py-3 px-4 bg-white/5 rounded-xl">
                    <div className="flex items-center space-x-3">
                      <div className={`w-3 h-3 rounded-full ${contact.status === 'completed' ? 'bg-green-500' :
                    ...
                    <div className="text-right">
                      <p className={`text-sm font-medium ${contact.status === 'completed' ? 'text-green-400' :
      ...
      {/* Welcome Message */}
      {!currentCampaign && (
        <div className="bg-gradient-to-br from-[#c4d82e]/10 to-[#c4d82e]/5 border border-[#c4d82e]/20 rounded-3xl p-8 text-center">
          <Phone className="w-16 h-16 text-[#c4d82e] mx-auto mb-4" />
          <h3 className="text-xl font-semibold text-white mb-2">
            Central de Ligações
          </h3>
          <p className="text-gray-400 mb-4">
            Configure seus agentes VAPI e gerencie suas campanhas de ligações automatizadas.
          </p>
          <p className="text-sm text-gray-500">
            Acesse a aba <strong className="text-[#c4d82e]">Agentes</strong> para começar.
          </p>
        </div>
      )}
    </div>
