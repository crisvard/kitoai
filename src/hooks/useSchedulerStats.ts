  ...
  useEffect(() => {
    ...
    const loadStats = async () => {
      try {
        ...
        // 400 pode ocorrer se a coluna franchise_id ainda não existir no banco (migration pendente)
        if (activeCustomersResult.error?.code === 'PGRST200' || activeCustomersResult.status === 400) {
          console.warn('⚠️ [useSchedulerStats] Coluna franchise_id não encontrada em customers — execute a migration pendente.');
          activeCustomersResult.count = null;
        }
        ...
        // Handle possible 400 error if columns are missing (pending migration)
        if (totalProfessionalsResult.error) {
          console.warn('⚠️ [useSchedulerStats] Erro ao contar profissionais:', totalProfessionalsResult.error.message);
          totalProfessionalsResult.count = null;
        }
