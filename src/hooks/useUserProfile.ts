...
export const useUserProfile = () => {
  ...
  // Setup Realtime Subscription to auto-refresh profile on DB changes
  useEffect(() => {
    if (!user) return;
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          console.log('⚡ [PROFILE-REALTIME] Recebido update do banco!', payload);
          // Set force refresh to ignore cache and fetch new data
          setForceRefresh(true);
        }
      )
      .subscribe();

    // Background refresh on window focus (SWR pattern)
    const onWindowFocus = async () => {
      console.log('🪟 [PROFILE-VISIBILITY] Janela focada, atualizando perfil silenciosamente...');
      try {
        const { data } = await supabase.from('profiles').select('*').eq('id', user.id).single();
        if (data) {
          setProfile((prev) => {
            if (!prev) return prev;
            if (prev.credits !== data.credits || prev.payment_status !== data.payment_status) {
              console.log('♻️ [PROFILE-BG-UPDATE] Mudança detectada no background!');
              // Limpa cache para próximos reloads pegarem o novo valor
              clearUserCache(user.id);
              return { ...prev, credits: data.credits, payment_status: data.payment_status };
            }
            return prev;
          });
        }
      } catch (e) {
        // Ignores error in background refresh
      }
    };
