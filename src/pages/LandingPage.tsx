...
import { useLandingServices, LandingService } from '../hooks/useLandingServices';
...
const LandingPage: React.FC<LandingPageProps> = ({ onBack }) => {
  const { landingPages, loading, createLandingPage, deleteLandingPage } = useLandingServices();
  const { plans } = usePlans();
  const [showAddModal, setShowAddModal] = useState(false);
  const [selectedLandingPage, setSelectedLandingPage] = useState<LandingService | null>(null);
  const [detailsTab, setDetailsTab] = useState<'details' | 'notes' | 'payment'>('details');
  const [formData, setFormData] = useState({ site_name: '', segment: '', site_link: '', is_active: false, is_trial: true });
  const [verifyingPayment, setVerifyingPayment] = useState(false);

  // Encontrar o plano de landing page — prioriza pelo ID específico
  const landingPlan = plans?.find((plan: any) => plan.id === 'landing');
  ...
  const handleVerifyPayment = async () => {
    if (!selectedLandingPage?.payment_id) {
      alert('ID do pagamento não encontrado para esta landing page.');
      return;
    }
    ...
    try {
      console.log('🔍 [LANDING_VERIFY] Verificando pagamento da landing page:', selectedLandingPage.id);
      ...
      const { data: statusData, error: statusError } = await supabase.functions.invoke('verify-payment-status', {
        body: { paymentId: selectedLandingPage.payment_id }
      });

      if (statusError) {
        console.error('❌ [LANDING_VERIFY] Erro na verificação:', statusError);
        alert('Erro ao verificar status do pagamento. Tente novamente.');
        return;
      }

      if (statusData.status === 'RECEIVED') {
        console.log('✅ [LANDING_VERIFY] Pagamento confirmado!');
        ...
        const { error: paymentUpdateError } = await supabase
          ...
          .eq('external_payment_id', selectedLandingPage.payment_id);

        if (paymentUpdateError) {
          console.warn('⚠️ [LANDING_VERIFY] Erro ao atualizar tabela payments:', paymentUpdateError);
        }
        ...
        const { error: updateError } = await supabase
          ...
          .eq('id', selectedLandingPage.id);

        if (updateError) {
          console.error('❌ [LANDING_VERIFY] Erro ao ativar landing page:', updateError);
          alert('Erro ao ativar a landing page. Tente novamente.');
          return;
        }

        // Atualizar o estado local
        setSelectedLandingPage(prev => prev ? { ...prev, is_active: true, activation_date: new Date().toISOString() } : null);

        alert('✅ Pagamento confirmado! Landing page ativada com sucesso.');
      } else {
        console.log('⏳ [LANDING_VERIFY] Pagamento ainda pendente:', statusData.status);
        alert('Pagamento ainda não foi confirmado. Tente novamente em alguns instantes.');
      }

    } catch (err: any) {
      console.error('❌ [LANDING_VERIFY] Erro ao verificar pagamento:', err);
  ...
  const handleAddLandingPage = async () => {
    if (!formData.site_name.trim()) {
      alert('Nome do App é obrigatório');
      return;
    }
    try {
      await createLandingPage({
        ...
        is_active: false,
        is_trial: true,
        notes: `Obrigado pelo cadastro! Aguarde o contato de um desenvolvedor para iniciar seu projeto. Você receberá nosso retorno em até 48 horas.

Nesta aba, após a contratação, você verá as informações de seu projeto, como: link de testes para validação, link da aplicação na Google Play e Apple Store, além de dados vitais da aplicação e contrato de desenvolvimento.

Atenciosamente, Kito Expert`
      });
      setShowAddModal(false);
      setFormData({ site_name: '', segment: '', site_link: '', is_active: false, is_trial: true });
      alert('App adicionado! Faça o pagamento para ativar.');
    } catch (err) {
      alert(err instanceof Error ? err.message : 'Erro ao adicionar landing page');
    }
  };
  ...
  const handleDeleteLandingPage = async (id: string) => {
    ...
    try {
      ...
      if (selectedLandingPage?.id === id) setSelectedLandingPage(null);
      alert('Landing page deletada!');
    } catch (err) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#1a1a1a] to-[#0f0f0f] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-white">Carregando landing pages...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#1a1a1a] to-[#0f0f0f]">
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-gray-400 text-sm mt-1">
                  Total: <span className="font-bold text-[#c4d82e]">{landingPages.length}</span>
                </p>
              </div>
            </div>
            <button
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        {!selectedLandingPage ? (
          ...
          <div>
            {landingPages.length === 0 ? (
              <div className="text-center py-12">
                ...
                <h3 className="text-xl font-bold text-white mb-2">Nenhuma landing page registrada</h3>
                <p className="text-gray-400 mb-6">Clique em "Nova Landing Page" para começar</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {landingPages.map(landingPage => (
                  <div
                    key={landingPage.id}
                    onClick={() => setSelectedLandingPage(landingPage)}
                  ...
                  >
                    ...
                    <div className="p-6 relative z-5">
                      ...
                      <h3 className="text-lg font-bold text-white mb-2 truncate group-hover:text-[#c4d82e] transition-colors duration-300">
                        {landingPage.site_name}
                      </h3>

                      {/* Link */}
                      {landingPage.site_link && (
                        <div className="flex items-center gap-2 text-sm text-gray-400 mb-4 truncate group-hover:text-gray-300 transition-colors duration-300">
                          ...
                          <span className="truncate text-xs">{landingPage.site_link}</span>
                        </div>
                      )}

                      {/* Badges */}
                      <div className="flex flex-wrap gap-2 mb-4">
                        {landingPage.domain_login && (
                        ...
                        {(landingPage as any).github_link && (
                        ...
                        {(landingPage as any).hosting_data && (
                      ...
                      <div className="mt-4 pt-4 border-t border-gray-700 text-xs text-gray-500 group-hover:text-gray-400 transition-colors duration-300 flex items-center gap-2">
                        ...
                        Criada em {new Date(landingPage.created_at).toLocaleDateString('pt-BR')}
                      </div>
          ...
          // Detalhes da Landing Page Selecionada - Layout melhorado
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              ...
              {!selectedLandingPage.is_active && (
                <button
                  onClick={() => handleDeleteLandingPage(selectedLandingPage.id)}
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[#2a2a2a] rounded-xl border border-gray-800 p-8 max-w-md w-full">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-white font-bold mb-2">Nome do App</label>
                <input
                  type="text"
                  value={formData.site_name}
                  onChange={e => setFormData({ ...formData, site_name: e.target.value })}
                  placeholder="Ex: Academia Brutal"
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Ex: Academia, Escritório, Clínica..."
                  className="w-full bg-[#1a1a1a] border border-gray-700 rounded-lg px-4 py-3 text-white placeholder-gray-500 focus:border-[#c4d82e] focus:outline-none"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Ex: Android, iOS, Ambos..."
                  className="w-full bg-[#1a1a1a] border border-gray-700 rounded-lg px-4 py-3 text-white placeholder-gray-500 focus:border-[#c4d82e] focus:outline-none"
                />
              </div>
...
function SelectedLandingPageView({
  selectedLandingPage,
  detailsTab,
  setDetailsTab,
  landingPlan,
  ...
  selectedLandingPage: LandingService;
  ...
  const activationDate = useMemo(() => {
    return selectedLandingPage.activation_date || selectedLandingPage.created_at;
  }, [selectedLandingPage]);

  const paymentMethod = selectedLandingPage.payment_method || '';
  const paymentId = selectedLandingPage.payment_id || '';
              ...
              <div className="min-w-0">
                ...
                {selectedLandingPage.is_active ? (
                  <div className="mt-1 inline-flex items-center gap-2 bg-green-500/15 border border-green-500/30 px-2.5 py-1 rounded-full">
                    ...
                    <span className="text-green-300 text-xs font-bold">Ativa</span>
                  </div>
                ) : (
        ...
        <div className="relative p-6 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-3 pt-4 border-t border-gray-700">
            <div className="rounded-xl border border-gray-800 bg-[#1a1a1a] p-4 transition-all duration-300 hover:border-[#c4d82e]/40">
              <div className="text-gray-500 text-[11px] font-bold uppercase">Criada em</div>
              <div className="text-white text-sm font-semibold mt-1">
                {new Date(selectedLandingPage.created_at).toLocaleDateString('pt-BR')}
              </div>
            </div>

            <div className="rounded-xl border border-gray-800 bg-[#1a1a1a] p-4 transition-all duration-300 hover:border-[#c4d82e]/40">
              <div className="text-gray-500 text-[11px] font-bold uppercase">Ativada em</div>
              <div className="text-white text-sm font-semibold mt-1">
                ...
                <div className="group relative bg-[#1a1a1a] rounded-2xl border border-gray-800 overflow-hidden transition-all duration-500 hover:border-[#c4d82e]/70 hover:shadow-xl hover:shadow-[#c4d82e]/10">
                  ...
                  <div className="relative p-5">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        <div className="text-gray-500 text-[11px] font-bold uppercase">Nome do App</div>
                        <div className="text-white font-bold truncate">{selectedLandingPage.site_name}</div>
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="relative p-5">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        {selectedLandingPage.segment ? (
                          <div className="text-white font-bold truncate">{selectedLandingPage.segment}</div>
                        ) : (
                          <div className="text-gray-400 font-semibold">—</div>
                        )}
                      </div>
              ...
              <div className="relative p-6">
                ...
                <div className="rounded-xl border border-gray-800 bg-[#0f0f0f]/30 p-4">
                  <div className="text-gray-300 whitespace-pre-wrap">
                    {((selectedLandingPage as any).notes as string | undefined) || '— Nenhuma nota registrada —'}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              {!selectedLandingPage.is_active ? (
                <div className="space-y-4">
                    ...
                    <div className="relative p-5">
                      <div className="flex items-center justify-between gap-4">
                        <div>
                          ...
                          <div className="text-white font-bold">Este App está pendente de pagamento</div>
                          <div className="text-gray-400 text-sm mt-1">Pagamento único, ativação individual por App.</div>
                        </div>
                        <button
                          onClick={() => {
                            window.location.href = `/direct-payment?plan=app-developer-plan&landingPageId=${selectedLandingPage.id}`;
                          }}
                  ...
                  {selectedLandingPage.payment_id && (
                    ...
                    <div className="relative p-5">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-white font-bold">{landingPlan ? formatPrice(landingPlan.price || landingPlan.monthly_price) : 'R$ 149,00'}</div>
