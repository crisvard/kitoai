...
const SchedulerContent: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  const { currentView, setCurrentView, setSettingsTab } = useScheduler();
  const { todayAppointments, totalAppointments, activeCustomers, totalProfessionals, loading: statsLoading } = useSchedulerStats();
  ...
  const handleAppointmentClickFromCalendar = (appointment: AppointmentFromCalendar) => {
    ...
    setSelectedDateFromCalendar(null);  // Limpa filtro de data
    setCurrentView('appointments');
  };

  const handleDateClickFromCalendar = (date: Date) => {
    setSelectedDateFromCalendar(date);
    setSelectedAppointmentFromCalendar(null);  // Limpa filtro de agendamento específico
    setCurrentView('appointments');
  };
      ...
      case 'dashboard':
        return (
          <div className="space-y-8">
            <Stats onReportsClick={handleReportsClick} onProfessionalsClick={handleProfessionalsClick} onServicesClick={handleServicesClick} onAppointmentsClick={handleAppointmentsClick} />
            <Calendar 
          ...
          <div className="xl:col-span-3">
            ...
            <div className="mb-8">
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  <h1 className="text-3xl font-bold text-white">Agente de WhatsApp</h1>
                  <p className="text-gray-400">Gerencie suas conversas e automatizações WhatsApp</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-white/5 to-white/[0.02] backdrop-blur-xl border border-white/10 rounded-3xl p-6 mb-6 shadow-2xl">
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                {tabs.map((tab, index) => {
                  ...
                  const isHovered = false; // Não temos hover state aqui
              ...
              <div className="p-6 border-b border-white/10">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-400">{statsLoading ? '...' : `${todayAppointments} agendamento${todayAppointments !== 1 ? 's' : ''}`}</p>
                  </div>
                </div>
              </div>
    ...
    <SchedulerAuthProvider>
      <AuthProvider>
        <ProfessionalAuthProvider>
          <SchedulerProvider>
            <SchedulerContent onBack={onBack} />
          </SchedulerProvider>
        </ProfessionalAuthProvider>
      </AuthProvider>
    </SchedulerAuthProvider>
