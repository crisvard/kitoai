...
const TrialLigacoesPage: React.FC = () => {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (profile?.ligacoes_trial_completed) {
      navigate('/direct-payment?reason=trial_used&plan=ligacoes');
    }
  }, [profile?.ligacoes_trial_completed, navigate]);
  ...
  const handleStartTrial = async () => {
    ...
    try {
      // Usar Edge Function para iniciar trial de ligações
      const { data: trialData, error: trialError } = await supabase.functions.invoke('start-trial-ligacoes', {
        body: { trialDays }
      });

      if (trialError) {
        throw new Error(trialError.message || 'Erro ao iniciar trial');
      }

      if (!trialData?.success) {
        throw new Error(trialData?.error || 'Falha ao iniciar trial');
      }

      console.log('✅ Trial de ligações iniciado:', trialData);
      ...
      window.location.href = '/dashboard?from_trial=true&service=ligacoes';
    } catch (err: any) {
      console.error('Erro ao iniciar trial de Ligações:', err);
      setError(err.message || 'Erro ao iniciar período de teste. Tente novamente.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#1a1a1a] to-[#0f0f0f] py-12 px-4">
      <div className="max-w-2xl mx-auto">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400">Agente de Ligações - Teste Gratuito por {trialDays} Dias</p>
        </div>
...
export default TrialLigacoesPage;
