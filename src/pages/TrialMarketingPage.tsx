...
const TrialMarketingPage: React.FC = () => {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (profile?.marketing_trial_completed) {
      navigate('/direct-payment?reason=trial_used&plan=marketing');
    }
  }, [profile?.marketing_trial_completed, navigate]);
  ...
  const handleStartTrial = async () => {
    ...
    try {
      ...
      const { error: updateError } = await supabase
        .from('profiles')
        .update({
          trial_marketing_active: true,
          trial_marketing_end_date: calculatedTrialEndDate.toISOString(),
          marketing_active: true, // Libera acesso durante trial
        })
        .eq('id', user.id);
      ...
      window.location.href = '/dashboard?from_trial=true&service=marketing';
    } catch (err: any) {
      console.error('Erro ao iniciar trial do Marketing:', err);
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#1a1a1a] to-[#0f0f0f] py-12 px-4">
      <div className="max-w-2xl mx-auto">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400">Agente de Marketing - Teste Gratuito por {trialDays} Dias</p>
        </div>
...
export default TrialMarketingPage;
