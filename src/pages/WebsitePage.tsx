...
const WebsitePage: React.FC<WebsitePageProps> = ({ onBack }) => {
  // OCULTADO TEMPORARIAMENTE: Verificação de bloqueio desabilitada
  // const isBlocked = profile?.website_access_blocked;
  // if (isBlocked) {
  //   return <div>Agente bloqueado</div>;
  // }
                        ...
                        <button
                          onClick={() => {
                            try {
                              ...
                              const websiteType = params.get('websiteType');
                              if (websiteType === 'lawyer') {
                                // Para websites de advogado, abrir a seleção de planos específica
                                window.location.href = `/lawyer-websites?websiteId=${selectedWebsite.id}`;
                              } else {
