...
export default function Calendar({ onAppointmentClick, onDateClick, onProfessionalFilter, isAdminView = false, showAppointmentDetails = true, franchiseId }: CalendarProps) {
  ...
  const loadAppointments = async () => {
    ...
    if (currentProfessional && !isAdminViewMode && !selectedProfessional) {
  ...
  const loadProfessionals = async () => {
    try {
      ...
      // Filter by franchise when in franchise view
      if (franchiseId) {
        query = query.eq('franchise_id', franchiseId);
      }
  ...
  return (
    <div className="bg-gradient-to-br from-white/5 to-white/[0.02] backdrop-blur-xl border border-white/10 rounded-3xl p-6" style={{ minHeight: '480px' }}>
        ...
        <div className="flex items-center space-x-3">
          ...
          <div className="bg-[#1a1a1a] border border-gray-600 rounded-2xl p-1 flex shadow-lg">
            <button
              ...
              className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all duration-300 transform hover:scale-105 ${viewMode === 'daily'
            ...
            <button
              ...
              className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all duration-300 transform hover:scale-105 ${viewMode === 'weekly'
                ? 'bg-gradient-to-r from-[#c4d82e] to-[#b8d025] text-black shadow-lg scale-105'
                : 'text-gray-400 hover:text-white hover:bg-[#2a2a2a]'
                }`}
            >
            ...
            <button
              ...
              className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all duration-300 transform hover:scale-105 ${viewMode === 'monthly'
                ? 'bg-gradient-to-r from-[#c4d82e] to-[#b8d025] text-black shadow-lg scale-105'
                : 'text-gray-400 hover:text-white hover:bg-[#2a2a2a]'
                }`}
            >
      ...
      {/* Sub-filtro por profissional — visível apenas em Dia e Semana, e somente no modo admin */}
      {isAdminView && (viewMode === 'daily' || viewMode === 'weekly') && professionals.length > 0 && (
        <div className="flex items-center gap-3 mb-5 p-3 bg-[#1a1a1a]/60 border border-white/10 rounded-2xl">
          <span className="text-xs text-gray-400 font-medium whitespace-nowrap">Profissional:</span>
          <select
            value={selectedProfessional || ''}
            onChange={(e) => {
              const value = e.target.value;
              setSelectedProfessional(value || null);
              if (onProfessionalFilter) onProfessionalFilter(value || null);
            }}
            className="flex-1 px-3 py-2 bg-[#2a2a2a] border border-gray-600 rounded-xl text-white text-sm focus:ring-2 focus:ring-[#c4d82e] focus:border-transparent transition-all duration-300 hover:border-gray-500"
          >
          ...
          {selectedProfessional && (
            <button
              onClick={() => { setSelectedProfessional(null); if (onProfessionalFilter) onProfessionalFilter(null); }}
              className="text-xs text-gray-400 hover:text-white px-2 py-1 rounded-lg hover:bg-white/10 transition whitespace-nowrap"
            >
              ✕ Limpar
            </button>
          )}
            ...
            {getDaysInMonth().map((day, index) => {
              ...
              return (
                <div
                  ...
                  className={`aspect-square bg-gradient-to-br from-white/5 to-white/[0.02] backdrop-blur-xl border rounded-2xl p-3 transition-all duration-300 cursor-pointer transform hover:scale-105 ${isToday
                ...
                >
                  <div className={`text-sm font-bold mb-2 ${isToday ? 'text-[#c4d82e]' : 'text-white'
                    }`}>
                    {day}
                  </div>
            ...
            appointments.map((apt) => (
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  ...
                  <span className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wide ${apt.status === 'confirmed' ? 'bg-green-500/20 text-green-400 border border-green-500/30' :
