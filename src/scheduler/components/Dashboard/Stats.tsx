...
export default function Stats({ onReportsClick, onProfessionalsClick, onServicesClick, onAppointmentsClick, franchiseId }: { onReportsClick?: () => void; onProfessionalsClick?: () => void; onServicesClick?: () => void; onAppointmentsClick?: () => void; franchiseId?: string }) {
  ...
  const getCacheKey = () => `scheduler_stats_${user?.id || 'anon'}_${franchiseId || 'all'}_${periodFilter}`;

  const loadCachedStats = (): StatsData | null => {
    try {
  ...
  const [stats, setStats] = useState<StatsData>(() => {
    // Try to restore from cache immediately so no layout shift on reload
    const cached = (() => {
      try {
        const key = `scheduler_stats_${(window as any).__schedulerUserId || 'anon'}_${franchiseId || 'all'}_month`;
        const raw = localStorage.getItem(key);
        return raw ? JSON.parse(raw) : null;
      } catch { return null; }
    })();
    return cached ?? { totalAppointments: 0, totalRevenue: 0, projectedRevenue: 0, professionalCount: 0, serviceCount: 0 };
  });
  const [loading, setLoading] = useState(() => loadCachedStats() === null);
  ...
  const loadStats = async () => {
    ...
    try {
      ...
      try { localStorage.setItem(getCacheKey(), JSON.stringify(newStats)); } catch { }
    } catch (err) {
