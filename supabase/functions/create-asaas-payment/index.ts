...
serve(async (req) => {
  ...
  try {
    ...
    // Buscar dados do plano ou pacote de créditos
    let plan = null;
    let isCreditPackage = false;

    // Tentar buscar na tabela de planos (agora unificada)
    ...
    if (planData) {
      plan = {
        ...planData,
        monthly_price: planData.price || planData.monthly_price,
        annual_price: planData.price || planData.annual_price
      };
      isCreditPackage = planId.startsWith('credits_');
      console.log('📋 [CREATE-PAYMENT] Plano/Pacote encontrado:', {
        name: plan.name,
        price: plan.price,
    ...
    if (!plan) {
      console.error('❌ [CREATE-PAYMENT] Plano/Pacote não encontrado:', planId)
      return new Response(
        JSON.stringify({ error: 'Plano ou pacote não encontrado' }),
    ...
    const description = isCreditPackage ? `Créditos Dialer - ${plan.name}` : `${plan.name} - ${isMonthly ? 'Mensal' : 'Anual'}`
    const externalReference = isCreditPackage ? `credits_${user.id}_${planId}_${Date.now()}` : `${user.id}_${planId}_${Date.now()}`
    ...
    const paymentData = {
      ...
      externalReference: externalReference, // Referência única
